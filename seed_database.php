...
try {
    ...
    // Mapping jenjang D-3/D-4 ke enum D3/D4
    $prodis = [
    ...
    // Data ini panjang, saya ambil logika: Nim 2 digit awal = Angkatan
    $mhsRaw = [
    ...
    // Semua MK ini untuk Prodi 7 (Teknik Telekomunikasi) berdasarkan Kurikulum ID 2 di dump lama
    $mks = [
    ...
    // === 9. RUANGAN (Dummy karena tabel lama tidak punya) ===
    ...
    // === 10. JADWAL KELAS (Konversi dari kelas_dosen) ===
    // Format: klsdsnId => [klsNama, Dosen, MK]
    $jadwals = [
    ...
    foreach($jadwals as $jadwal) {
        Capsule::table('jadwal_kelas')->insert([
            'id' => $jadwal['id'], // Keep ID same as old DB for KRS mapping
    ...
    // === 11. KRS (Logic: Mahasiswa di Kelas X mengambil semua Jadwal Kelas X) ===
    // Data Kelas Mahasiswa (78 Data)
    $kelasMhs = [
    ...
    foreach($kelasMhs as $group) {
        // Ambil semua ID jadwal untuk kelas ini
        $jadwalIds = Capsule::table('jadwal_kelas')
            ->where('nama_kelas', $group['kls'])
            ->pluck('id')
            ->toArray();
        
        // Loop setiap mahasiswa di kelas tersebut
